...
char* rando() {
	...
	char arr[N];
	int i, j, len;
	char obj[] = "QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm";
...
int BFS(Graph* graph) 
{
	...
	while (queue->head) 
	{
		...
		while (ptr2) 
		{
			...
			{
				ptr2->top->color = 1;
				ptr2->top->d = f->v->d + ptr2->weight;//w_bet(graph, start,ptr->top);
...
int Algorythm_Prima(Graph* graph)
{
	...
	for (int g = 0; g < graph->now; g++) 
	{
		/*if (graph->mas[g]->key == 0.0)
			continue;*/
		Edge* ptr = graph->mas[g]->next2;
		while (ptr)
		{
			if (ptr->weight == graph->mas[g]->key)//&&ptr->top->parant == graph->mas[g])//?? 
			{
				//2Vertex* pp = 
				printf("Hello ");
