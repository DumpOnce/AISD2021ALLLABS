...
реализованной в виде двоичного идеально сбалансированного дерева поиска
(для любого узла количество узлов в правом и левом поддеревьях отличается не более чем на 1). 
Узел дерева содержит ключ, указатели на правое и левое поддеревья и указатель на информационное поле. 
В узел дерева могут быть включены дополнительные поля для подержания балансировки (для хранения коэффициента перевеса).
